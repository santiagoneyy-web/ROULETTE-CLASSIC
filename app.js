...
async function requestAutoAI() {
    ...
    try {
        ...
        let modeInstruction = window.currentAIMode === 'SAFE' ? 'Si no hay patr�n, responde "ESPERAR".' : 'PROHIBIDO decir ESPERAR. MODO FULL: DEBES DAR PREDICCI�N SIEMPRE OBLIGATORIAMENTE.';
        const p = `Eres un motor estad�stico de ruleta en vivo. Historial: ` + history.slice(-15).join(',') + `. Da EXACTAMENTE dos predicciones MUY BREVES (max 3 palabras c/u). 1) Jugada amplia (N9). 2) Jugada agresiva (N4). ` + modeInstruction + ` Formato EXACTO: "N9: Jugar al X | N4: Jugar al Y" o "N9: ESPERAR | N4: ESPERAR".`;
        ...
        if (data.reply) {
            ...
            n9El.innerText = pN9;
            n4El.innerText = pN4;
            if (analysisEl) analysisEl.innerText = `An�lisis completado para el �ltimo giro (` + history[history.length-1] + `).`;
