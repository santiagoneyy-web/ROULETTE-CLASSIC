...
function analyzeTravelPattern(hist) {
    ...
    const getStr = (state, type) => {
        if (state.startsWith('S:')) return type === 'dir' ? `Dir ${state.split(':')[1]} S�lida` : `Zona ${state.split(':')[1]} S�lida`;
...
async function requestAutoAI() {
    ...
    try {
        ...
        try {
            ...
            let pat = (typeof analyzeTravelPattern === "function") ? analyzeTravelPattern(history) : {label:"Est�ndar",tiradas:0};
            ...
            stabilityInfo = `ESTADO DE LA MESA: ${colorNames[lvl]} | PATR�N: ${pat.label}`;
...
`;
        ...
        let modeInstruction = window.currentAIMode === 'SAFE' ? 'Si la mesa NO est� en VERDE o no hay patr�n claro, responde "ESPERAR".' : 'PROHIBIDO decir ESPERAR. Elige los mejores numeros basandote en el momentum.';
...
INSTRUCCION: Utiliza el COLOR de estabilidad para decidir el nivel de riesgo. Si es VERDE, conf�a m�s en las medidas. Si es ROJO, busca cambios de tendencia.
...
Formato EXACTO: "N9: Jugar al X | N4: Jugar al Y" o "N9: ESPERAR | N4: ESPERAR".`;
        ...
        if (data.reply) {
            ...
            n9El.innerText = pN9;
            n4El.innerText = pN4;
            if (analysisEl) analysisEl.innerText = `An�lisis [${stabilityInfo.split('|')[0]}]: CW vs CCW`;
